    ...
    return __awaiter(this, void 0, void 0, function* () {
        ...
        try {
            ...
            if (isIncremental) {
                ...
                lib_1.logger.debug(`New results in ${newResultsJson[0]}`);
                lib_1.logger.debug(`Read file...`);
                const newResultsContent = yield fs_1.default.readFileSync(newResultsJson[0]);
                lib_1.logger.debug(`a`);
                const newResults = JSON.parse(newResultsContent.toString());
                lib_1.logger.debug(`b`);
                // TODO validate file exists and is .json?
                const jsonV7Content = yield fs_1.default.readFileSync(resultsJson[0]);
                lib_1.logger.debug(`c`);
                const coverityIssues = JSON.parse(jsonV7Content.toString());
                lib_1.logger.debug(`d`);
                issuesUnified = new Array();
                for (const issue of coverityIssues.issues) {
                    for (const newResult of newResults) {
                        lib_1.logger.debug(`1`);
            ...
            lib_1.logger.info(`Found ${issuesUnified.length} Reported Polaris issues.`);
            let security_gate_pass = true;
            if (securityGateFilters) {
                lib_1.logger.debug(`Checking security gate...`);
                for (const issue of issuesUnified) {
                    lib_1.logger.debug(`Test issue ${issue.key} severity: ${issue.severity} cwe: ${issue.cwe}`);
