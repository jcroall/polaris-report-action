...
function readSecurityGateFiltersFromString(securityGateString) {
    ...
    let securityGateMap = new Map();
    lib_1.logger.debug(`Reading security gate filters`);
    for (const key in securityGateJson) {
        if (Object.hasOwnProperty(key)) {
            lib_1.logger.debug(`  ${key} : ${securityGateJson[key]}`);
        }
    }
    process.exit(1);
...
function run() {
    return __awaiter(this, void 0, void 0, function* () {
        ...
        readSecurityGateFiltersFromString(inputs_1.SECURITY_GATE_FILTERS);
